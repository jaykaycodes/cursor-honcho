...
After exploring the codebase, clarify requirements **in chat** (Cursor has no Claude Code `AskUserQuestion` tool). Ask **one topic at a time** with short labeled options the user can reply to.

#### Question Set 1 — Entities & peers

Which entities should Honcho track and build representations for? (Adapt options to what you found, e.g. end users only, users + AI assistant, multiple AI agents, third-party services.) If multiple AI agents exist, follow up: should any AI peers be observed?

#### Question Set 2 — Integration pattern

How should the AI access Honcho user context?

- **Tool call (recommended)** — on-demand via function / MCP tools
- **Pre-fetch** — load context before each model call with fixed queries
- **context()** — conversation history + representations in the prompt
- **Multiple** — combine approaches

#### Question Set 3 — Session structure

How should conversations map to Honcho sessions? (e.g. one session per chat thread, per user, group chat, custom.)

#### Question Set 4 — Pre-fetch detail (if applicable)

If they chose pre-fetch, ask which context dimensions matter (they can pick several): communication style, expertise, goals, preferences, recent activity summary, custom queries.
