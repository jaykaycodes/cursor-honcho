...
/** Map Cursor (and other) agent tool names to the Claude-style shape used by summaries. */
function normalizeToolHookPayload(hookInput: HookInput): HookInput {
  const rawName = hookInput.tool_name || "";
  ...
  if (key === "write") {
    return {
      ...
      tool_name: "Write",
      tool_input: {
        file_path: input.target_file ?? input.path ?? input.file_path,
        content: input.contents ?? input.content ?? input.text ?? "",
      },
    };
  }
  if (key === "search_replace" || key === "str_replace") {
    return {
      ...hookInput,
      tool_name: "Edit",
      tool_input: {
        file_path: input.file_path ?? input.path ?? input.target_file,
        old_string: input.old_string ?? "",
        new_string: input.new_string ?? "",
      },
    };
  }
  if (key === "run_terminal_cmd" || key === "terminal_command") {
    return {
      ...hookInput,
      tool_name: "Bash",
      tool_input: {
        command: input.command ?? input.command_line ?? input.cmd ?? "",
      },
    };
  }
  if (key === "notebookedit" || key === "notebook_edit") {
    return {
      ...hookInput,
      tool_name: "NotebookEdit",
      tool_input: {
        notebook_path: input.notebook_path ?? input.path,
  ...
  if (key === "apply_patch") {
    return { ...hookInput, tool_name: "ApplyPatch", tool_input: {} };
  }
  return hookInput;
}
...
function shouldLogTool(toolName: string, toolInput: Record<string, any>): boolean {
  const significantTools = new Set([
    "Write", "Edit", "Bash", "Task", "NotebookEdit", "DeleteFile", "ApplyPatch",
  ]);
...
export async function handlePostToolUse(): Promise<void> {
  ...
  hookInput = normalizeToolHookPayload(hookInput);
