...
export function detectHost(stdinInput?: Record<string, unknown>): HonchoHost {
  ...
  if (stdinInput?.cursor_version != null) return "cursor";
  // Cursor sets CURSOR_PROJECT_DIR for child processes
  if (process.env.CURSOR_PROJECT_DIR) return "cursor";
  // Cursor Honcho plugin: default host is `cursor` (unlike Claude Code, which defaults to claude_code)
  return "cursor";
}
