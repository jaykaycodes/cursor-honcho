...
async function setup(): Promise<void> {
  // Cursor plugin: materialize defaults under hosts.cursor
  ...
  if (!apiKey) {
    ...
    console.log("  3. Restart Cursor and invoke the honcho-setup skill again");
    process.exit(1);
  }
  ...
  if (!configExists()) {
    ...
    try {
      ...
      // Per-host config goes in hosts.cursor via saveConfig
      saveConfig({
