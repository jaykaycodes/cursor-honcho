...
function handleGetConfig(cwd: string) {
  ...
  if (cfgExists && !rawFile.hosts) {
    warnings.push("Config uses legacy flat fields. Consider running the honcho-config skill to migrate to hosts block.");
  }
...
function handleSetConfig(args: Record<string, unknown>) {
  ...
  const restartWarning = SESSION_AFFECTING_FIELDS.has(field)
    ? "Close and restart Cursor. Open agent sessions may still use the previous config until you restart."
    : undefined;
